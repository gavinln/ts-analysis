...
def variableParser(escapedChars, baseChars=pp.alphanums):
    ...
    return pp.Combine(
        pp.Word(baseChars) +
        pp.Optional(pp.OneOrMore(escapeDef + pp.Word(baseChars))))
...
keyNameWithoutSpace = pp.variableParser('_-./').setParseAction(
    pp.downcaseTokens)
...
def paramParser(comment='#'):
    ...
    rawValue = pp.Combine(
        rawLine + pp.ZeroOrMore(
            pp.White(' \t').suppress() + pp.NotAny('[') + rawLine))
    rawValue.setParseAction(lambda t: [x.strip() for x in t])

    valueDef = pyValue | rawValue
    paramDef = keyName('name') + pp.Optional(unitDef)('unit') + pp.Suppress(
        "=" + pp.empty) + valueDef('value')
...
def tableColParser():
    ...
    tableCol = (
        ...
        unitLine('unit') +
        pp.Group(pp.OneOrMore(tabValueLine))('data')).setParseAction(
            formatBloc)
...
def matrixParser(pattern=pyValue):
    ...
    firstLine = pp.Group(pp.OneOrMore(pattern) + EOL).setParseAction(
        defineColNumber)
    matrixLine = pp.Forward()
    matrixDef = (firstLine + pp.OneOrMore(matrixLine)).setParseAction(
        formatBloc)
...
def tableRowParser():
    ...
    tabValueDef = pyValue | pp.CharsNotIn('[ \t\r\n').setWhitespaceChars(" \t")
    rowHeader = (keyName("header") +
                 unitDef('unit')) | keyNameWithoutSpace('header')
    firstLine = pp.Group(
        rowHeader + pp.Group(pp.OneOrMore(tabValueDef))('value') + EOL
    ).setParseAction(defineColNumber)
    tabValueLine = pp.Forward()

    tableRowDef = (firstLine +
                   pp.OneOrMore(tabValueLine)).setParseAction(formatBloc)
...
def test_pattern(pattern, fname='data.txt'):
    ...
    for r, s, t, in pattern.scanString(pp.file(fname).read()):
        print('found : ', r)
